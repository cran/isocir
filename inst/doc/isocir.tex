...
the methodology of \citet{Rue09} and \citet{Fer11}. The isocir package is illustrated in Section~\ref{sec4} using the motivating
...
\end{equation}
It is important to note that the order among the angular parameters is invariant of the location of the pole (the initial point
...
violators algorithm (PAVA) or the isotonic regression (see \citet{Rob88} for details) cannot be applied to circular data. For
...
$\phi_i$ is the modal direction $\kappa$ is the concentration parameter (see \citet{Mar00}). Under such an assumption,
...
$\hat\kappa$ is the estimator of $\kappa$, $\widetilde{\phi }$ is the CIRE computed under $H_{0}$. $\widetilde\phi$ determines a
partition of $\wp=\{1,\ldots,I\}$ into sets of coordinates on which $\widetilde\phi$ is constant. These sets are called level
sets. The rejection region for the conditional test is given by:
\begin{equation*}
...
\label{sec3} \label{sec3} We start this section giving some background on \proglang{R} packages for isotonic regression and
analysis of circular data, we then describe the structure of our package \pkg{isocir} and illustrate it by some examples.
...
\pkg{isocir}, for analyzing circular data under constraints. Our package uses \pkg{circular} (see \cite{circular}) and
...
For the convenience of the reader, we summarize all the functions, arguments and descriptions of our package isocir in Table
1.\\
...
\begin{itemize}
  ...
  \end{itemize}
The auxiliary function sce computes the sum of circular error between a given q-dimensional vector (denoted by arg1) and one or
more q-dimensional vectors (denoted by arg2). The function mrl computes the mean resultant length for the input "data".
\begin{itemize}
...
\hline \textbf{\code{bias}} & (NULL) & \code{=TRUE}(by default) / \code{=FALSE}\\
\hline
\end{tabular}
...
$\hat{\kappa}$ is shown in the output. The \code{bias} is related to the estimation of $\kappa$. If \code{bias=TRUE} the bias
...
\code{$pvalue}) but also an attribute called \code{estkappa} will inform (or rather remember) the user if the value in
...
is a matrix where each column contains the values of a replication and each row the angles observed at each population. In this
...
Moreover, we correct the bias in the estimation of $\kappa$, so we set \code{bias=TRUE}.  Thus we have the following code:
% CHUNK 7
\begin{Schunk}
\begin{Sinput}
...
(budding yeast) and \emph{S. Pombe} (fission yeast). Using the 10 published budding yeast data (\citet{Rus04}, \citet{Ol05},
\citet{Pen05}), we illustrate the isocir package to test the null hypothesis that 16 fission yeast genes, namely, \emph{ssb1},
...
\end{array}
\end{equation}
For each of the 10 experimental data, the unconstrained estimates of the phase angles of the above 16 fission yeast genes
...
Since the 10 experiments may not considered as replications of each other, we performed a separate test for each experiment.
Moreover, as explained in \citet{Fer11} we assume that the concentration parameter $\kappa_j$ depends on the experiment but not
on the gene. The reason for this is that out of the two sources of uncertainty, one specific to the gene and another one due to
the experiment (and therefore common to all genes within the experiment), the former source maybe considered negligible relative
to the latter as the number of time points used in each time course experiment is fairly large for any specific gene.

For each of the 10 experiments, we  test the hypothesis (\ref{hypo}) using our software \code{cond.test}. The $\kappa_j$ values
considered for this example are obtained using \citet{Fer11}. The procedure used for the computations of these values comes from
a mixed effects models approach for circular data and is fully detailed in the supplementary material of that paper. The
following code gives the p-values for each experiment. Results are summarized in Table~\ref{SCEyCIRE}.
% CHUNK 9 
\begin{Schunk}
\begin{Sinput}
>  data("cirgenes")
